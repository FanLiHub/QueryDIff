...
class DinoVisionTransformer(BaseModule):
    ...
    def forward(self, *args, **kwargs):
        ...
        # resize
        args[0] = F.interpolate(x, size=(new_H, new_W), mode='bilinear', align_corners=False)

        # 转回 tuple
        args = tuple(args)
        ret = self.forward_features(*args, **kwargs)
        from vis import feature_vis
        _vis = feature_vis(args[0], ret[0], '/datanvme0/lifan/debug/iccv/', 6, match_flag=False)
        _vis.vis_img(mode=1)
        # from torchvision import utils as vutils
        # import os
        # new_filename = 'ori.png'
        # base, ext = os.path.splitext(new_filename)
        # counter = 1
        # while os.path.exists(os.path.join('/datanvme0/lifan/debug/', new_filename)):
        #     new_filename = f"{base}_{counter}{ext}"
        #     counter += 1
        # vutils.save_image(args[0], '/datanvme0/lifan/debug/'+new_filename, normalize=True)
